...
function Product({ props }) {
  ...
  const clickVolume = (id) => {
    setActiveVolume(id);
  };

  return (
    <div className="product-item">
      <img className="product-item__img" src={props.imageUrl} alt="" />
      <div className="product-item__inner">
        <a className="product-item__link" href="#1">
          <p className="product-item__name">{props.name}</p>
        </a>
        <p className="product-item__descr">{props.descr}</p>
        <div className="product-item__data pi-data">
          <div className="pi-data__btns">
            ...
            <div className="pi-data__row">
              ...
              <div className="pi-data__row-content">
                {props.volumes.map((el, idx) => {
                  return (
                    <button
                      key={idx}
                      onClick={() => clickVolume(idx)}
                      className={`pi-data__btn hollow-btn ${
